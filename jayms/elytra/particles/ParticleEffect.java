package jayms.elytra.particles;
...
import jayms.elytra.particles.ReflectionUtil.PackageType;
...
public enum ParticleEffect {
	...
	private static final String[] validNames = new String[values().length];
	
	static {
		for (int i = 0; i < values().length; i++) {
			validNames[i] = values()[i].toString().toLowerCase();
		}
	}
	
	public static boolean isParticleName(String name) {
		return Arrays.asList(validNames).contains(name.toLowerCase());
	}
	
	public static ParticleEffect findParticleFromName(String name) {
		if (isParticleName(name)) {
